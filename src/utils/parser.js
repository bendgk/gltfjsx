...
function parse(gltf, { fileName = 'model', ...options } = {}) {
  ...
  const importModelPath = `import modelPath from './${fileName}';`
  const textureImports = options.external ?
    options.textures.map((texture) => `import Image${texture.name} from './textures/${texture.name}${texture.ext}';`).join('\n') :
    ''
  const threeImports = options.external ? 'import { MeshStandardMaterial } from "three";' : ''

  const materials = {}

  gltf.parser.associations.forEach((key, value) => {
    if (value.type === "MeshStandardMaterial") {
      const mat = gltf.parser.json.materials.filter((m) => m.name === value.name)[0]
  ...
  function matsToString(materials) {
    const matList = []
...
`.replaceAll(/"Texture(\d+)"/g, "Texture$1")
    return tmpl
  }

  const url = options.external ? 'modelPath' : (fileName.toLowerCase().startsWith('http') ? '' : '/') + fileName
...
${parseExtras(gltf.parser.json.asset && gltf.parser.json.asset.extras)}*/`
  const result = `${options.types ? `\nimport * as THREE from 'three'` : ''}
        ...
        ${options.external && options.types ? '// @ts-ignore' : ''}
        ${options.external ? importModelPath : ''}
        ${options.external ? textureImports : ''}
        ${options.external ? threeImports : ''}
        import { useGLTF, ${options.external ? `useTexture, ` : ``}${hasInstances ? 'Merged, ' : ''} ${
    scene.includes('PerspectiveCamera') ? 'PerspectiveCamera,' : ''
        ...
        export function Instances({ children, ...props }) {
          const { nodes } = useGLTF(${oq}${url}${oq}${options.draco ? `, ${JSON.stringify(options.draco)}` : ''})${
    ...
    !options.instanceall
      ? `const { nodes, ${options.external ? '' : 'materials'}${hasAnimations ? ', animations' : ''} } = useGLTF(${oq}${url}${oq}${
    ...
    ${
      options.external ?
        `const [
          ${options.textures.map((t) => `Texture${t.name}`).join(', ')}
        ] = useTexture([
          ${options.textures.map((t) => `Image${t.name}`).join(", ")}
        ])
        `
        : ``
    }
...
${options.external ? '' : `useGLTF.preload('${url}')`}`
